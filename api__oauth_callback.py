...
GITHUB_TOKEN = os.environ["GITHUB_TOKEN"]          # GitHub token with repo write access
...
def handler(event, context=None):
    try:
        ...
        if success:
            # Simple success page
            html = f"<html><body><h1>✅ Verified!</h1><p>User {user_id} added.</p></body></html>"
            return {"statusCode": 200, "headers": {"Content-Type": "text/html"}, "body": html}
        return {"statusCode": 500, "body": "⚠️ Failed to update GitHub."}
    except Exception as e:
        html = f"<html><body><h1>⚠️ Error</h1><p>{e}</p></body></html>"
